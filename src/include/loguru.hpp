...
loguru::Verbosity_MAX);
        ...
        VLOG_F(get_log_level(), "Use vlog for dynamic log level (integer in the
range 0-9, inclusive)");
        ...
        CHECK_EQ_F(a, b, "You can also supply a custom message, like to print
something: %d", a + b);
...
message.message);
        ...
        Before including <loguru.hpp> you may optionally want to define the
following to 1:
        ...
        LOGURU_WITH_STREAMS (default 0):
                Add support for _S versions for all LOG and CHECK functions:
                        ...
                        CHECK_EQ_S(a, b) << "I expected a and b to be the
same!";
        ...
        LOGURU_UNSAFE_SIGNAL_HANDLER (default 1):
                ...
                This may lead to bad things like deadlocks in certain
situations.
        ...
        LOGURU_USE_FMTLIB (default 0):
                ...
                You will need to link against `fmtlib` or use the
`FMT_HEADER_ONLY` preprocessor definition.
                Feature by kolis (https://github.com/emilk/loguru/pull/22)

        LOGURU_WITH_FILEABS (default 0):
                When LOGURU_WITH_FILEABS is defined, a check of file change will
be performed on every call to file_log.
                If the file is moved, or inode changes, file is reopened using
the same FileMode as is done by add_file.
                Such a scheme is useful if you have a daemon program that moves
the log file every 24 hours and expects new file to be created.
        ...
        LOGURU_RTTI (default 1):
                Set to 0 if your platform does not support runtime type
information (-fno-rtti).

        You can also configure:
        loguru::g_flush_interval_ms:
                If set to zero Loguru will flush on every line (unbuffered
mode).
                Else Loguru will flush outputs every g_flush_interval_ms
milliseconds (buffered mode).
                The default is g_flush_interval_ms=0, i.e. unbuffered mode.

# Notes:
        * Any arguments to CHECK:s are only evaluated once.
        * Any arguments to LOG functions or LOG_SCOPE are only evaluated iff the
verbosity test passes.
        * Any arguments to LOG_IF functions are only evaluated if the test
passes.
*/
...
/*  Should be called from the main thread.
   ...
   (default).
                        -v INFO     Only show INFO, WARNING, ERROR, FATAL
   (default).
...
void log(Verbosity verbosity, const char *file, unsigned line,
         LOGURU_FORMAT_STRING_TYPE format, fmt::ArgList args);
FMT_VARIADIC(void, log, Verbosity, const char *, unsigned,
             LOGURU_FORMAT_STRING_TYPE)

// Log without any preamble or indentation.
void raw_log(Verbosity verbosity, const char *file, unsigned line,
             LOGURU_FORMAT_STRING_TYPE format, fmt::ArgList args);
FMT_VARIADIC(void, raw_log, Verbosity, const char *, unsigned,
             LOGURU_FORMAT_STRING_TYPE)
...
/* Returns the thread name for this thread.
   ...
   right_align_hext_id means any hexadecimal thread id will be written to the
   end of buffer.
*/
...
/* 	A stack trace gives you the names of the function at the point of a
   crash.
        With ERROR_CONTEXT, you can also get the values of select local
   variables.
        Usage:
...
/*
   ...
   error context stack.
        ...
        This can be very useful for when you have a parent thread spawning
   several working thread,
        and you want the error context of the parent thread to get printed (too)
   when there is an
...
#define LOG_SCOPE_FUNCTION(verbosity_name)                                     \
    LOG_SCOPE_F(verbosity_name, __PRETTY_FUNCTION__)
...
#define CHECK_WITH_INFO_F(test, info, ...)                                     \
    ...
    ? (void)0 : loguru::log_and_abort(0, "CHECK FAILED:  " info "  ",          \
    ...
    do {                                                                       \
        ...
        if (!LOGURU_PREDICT_TRUE(val_left op val_right)) {                     \
            ...
            auto fail_info = loguru::textprintf(                               \
                "CHECK FAILED:  %s %s %s  (%s %s %s)  ", #expr_left, #op,      \
                #expr_right, str_left.c_str(), #op, str_right.c_str());        \
...
// ----------------------------------------------------------------------------
// 88 8b    d8 88""Yb 88     888888 8b    d8 888888 88b 88 888888    db
// 888888 88  dP"Yb  88b 88
// 88 88b  d88 88__dP 88     88__   88b  d88 88__   88Yb88   88     dPYb     88
// 88 dP   Yb 88Yb88
// 88 88YbdP88 88"""  88  .o 88""   88YbdP88 88""   88 Y88   88    dP__Yb    88
// 88 Yb   dP 88 Y88
// 88 88 YY 88 88     88ood8 888888 88 YY 88 888888 88  Y8   88   dP""""Yb   88
// 88  YbodP  88  Y8
    ...
    for (int arg_it = 1; arg_it < argc; ++arg_it) {
        ...
        if (strncmp(cmd, verbosity_flag, arg_len) == 0 &&
            ...
            if (strcmp(value_str, "OFF") == 0) {
                ...
                CHECK_F(end && *end == '\0',
                        "Invalid verbosity. Expected integer, "
                        "INFO, WARNING, ERROR or OFF, got '%s'",
                        value_str);
            }
        } else {
...
void get_thread_name(char *buffer, unsigned long long length,
    ...
    if (buffer[0] == 0) {
        ...
        if (right_align_hext_id) {
            snprintf(buffer, length, "%*X", length - 1,
...
static void print_preamble_header(char *out_buff, size_t out_buff_size) {
    ...
    snprintf(out_buff, out_buff_size,
             ""); // Make sure there is a '\0' and handle out_buff_size==0
    if (g_preamble_date) {
        ...
        if (g_colorlogtostderr && s_terminal_has_color) {
            if (verbosity > Verbosity_WARNING) {
                fprintf(stderr, "%s%s%s%s%s%s%s%s%s\n", terminal_reset(),
                        ...
                        terminal_reset(),
                        verbosity == Verbosity_INFO ? terminal_bold()
                                                    : terminal_light_gray(),
                        message.prefix, message.message, terminal_reset());
            } else {
                fprintf(stderr, "%s%s%s%s%s%s%s%s\n", terminal_reset(),
                        terminal_bold(),
                        verbosity == Verbosity_WARNING ? terminal_red()
                                                       : terminal_light_red(),
...
LogScopeRAII::~LogScopeRAII() {
    if (_file) {
        ...
        log(_verbosity, _file, _line, "} %.*f s: %s", SCOPE_TIME_PRECISION,
            duration_sec, _name);
    }
}
...
// Yb  dP 888888
// 88__   88__dP 88__dP dP   Yb 88__dP     dP   `" dP   Yb 88Yb88   88   88__
// YbdP    88
// 88""   88"Yb  88"Yb  Yb   dP 88"Yb      Yb      Yb   dP 88 Y88   88   88""
// dPYb    88
// 888888 88  Yb 88  Yb  YbodP  88  Yb      YboodP  YbodP  88  Y8   88   888888
// dP  Yb   88
// ----------------------------------------------------------------------------
